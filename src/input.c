...
struct alignment** detect_and_read_alignments(struct alignment** alignments,struct parameters* param)
{
        ...
        input = tmalloc(sizeof(char*) * num_alignments);
        input_type = tmalloc(sizeof(unsigned short int) * num_alignments);
        input_numseq = tmalloc(sizeof(unsigned short int) * num_alignments);
        ...
        for (i = 0; i < num_alignments;i++){
                ...
                if(input[i]){
                        if (byg_start("<macsim>",input[i]) != -1){
                                input_numseq[c] = count_sequences_macsim(input[i]);
                                input_type[c] = 1;
                        }else if (byg_start("<uniprot",input[i]) != -1){
                                input_numseq[c] = count_sequences_uniprot(input[i]);
                                input_type[c] = 2;
                        }else if(byg_start("This SWISS-PROT",input[i]) != -1){
                                input_numseq[c] = count_sequences_swissprot(input[i]);
                                input_type[c] = 3;
                        }else if (byg_start("This Swiss-Prot",input[i]) != -1){
                                input_numseq[c] = count_sequences_swissprot(input[i]);
                                input_type[c] = 3;
                        }else if (byg_start("CLUSTAL W",input[i]) != -1){
                                input_numseq[c] = count_sequences_clustalw(input[i]);
                                input_type[c] = 4;
                                ...
                                input_numseq[c] = count_sequences_stockholm(input[i]);
                                input_type[c] = 5;
                        }else{
                                input_numseq[c]  = count_sequences_fasta(input[i]);
                                input_type[c] = 0;
                        }
                        c++;
                        fprintf(stderr,"%s found %d sequences\n",param->infile[i],input_numseq[i]);
                        numseq += input_numseq[i];
                }else{
                        fprintf(stderr,"%s found no sequences ",param->infile[i]);
                        if(!param->outfile && i){
                                ...
                                /*if(!param->format){
                                  ...
                                  if(param->format){
                                  fprintf(stderr,"-> assuming %s format\n",param->format);
                                  }else{
                                  fprintf(stderr,"-> assuming fasta format\n");
                                  }
                                  }*/
                                num_alignments--;
        ...
        for (i = 0; i < num_alignments;i++){
                if(input[i]){
                        numseq = input_numseq[c];
                        numprofiles = numseq << 1;
                        alignments[c]  = aln_alloc(aln);
                        aln = alignments[c];
                        aln->numseq = input_numseq[c];
                        switch(input_type[c]){
                        case 0:
...
static struct alignment* read_alignment_from_swissprot(struct alignment* aln,char* string)
{
        ...
        while ((i = byg_end("ID   ",p)) != -1){
                ...
                j = byg_start(" ",p);
                aln->lsn[c] = j;
                aln->sn[c] = tmalloc(sizeof(char)*(j+1));
                for (i = 0;i < j;i++){
                        aln->sn[c][i] = p[i];
                }
                aln->sn[c][j] = 0;
                ...
                aln->s[c] = tmalloc(sizeof(int)*(j+1));
                aln->seq[c] = tmalloc(sizeof(char)*(j+1));
                n = 0;
                for (i = 0;i < j;i++){
                        if((int)p[i] > 32){
                                if(isalpha((int)p[i])){
...
static struct feature* read_ft(struct feature* ft,char* p)
{
        ...
        while((j = byg_end("<fitem>",p1))!= -1){
                ...
                n->type = tmalloc(sizeof(char*)*(j+1));
                for (i = 0; i < j;i++){
                        n->type[i] = p1[i];
                }
                n->type[j] = 0;
                ...
                n->note = tmalloc(sizeof(char*)*(j+1));
                for (i = 0; i < j;i++){
                        n->note[i] = p1[i];
                }
...
static struct alignment* read_alignment_uniprot_xml(struct alignment* aln,char* string)
{
        ...
        /*aln = (struct alignment *) tmalloc(sizeof(struct alignment));
          ...
          aln->si = 0;
          aln->ft = 0;
          aln->sl = tmalloc(sizeof(int) * (numprofiles));
          aln->sip = tmalloc(sizeof(int*)* numprofiles);
          aln->nsip = tmalloc(sizeof(int)* numprofiles);
          aln->sn = tmalloc(sizeof(char*) * numseq);
          aln->lsn = tmalloc(sizeof(int) * numseq);
...
static struct alignment* read_alignment_clustal(struct alignment* aln,char* string)
{
        ...
        for(i =start;i < local_numseq+start;i++){
                aln->s[i] = tmalloc(sizeof(int)*(len+1));
                aln->seq[i] = tmalloc(sizeof(char)*(len+1));
        }
...
static struct alignment* read_alignment(struct alignment* aln,char* string)
{
        ...
        for (i = start; i < local_numseq+start;i++){
                ...
                aln->s[i] = tmalloc(sizeof(int)*(aln->sl[i]+1));
                aln->seq[i] = tmalloc(sizeof(char)*(aln->sl[i]+1));
                aln->sn[i] = tmalloc(sizeof(char)*(aln->lsn[i]+1));
                //aln->sip[i] = tmalloc(sizeof(int)*1);
                //aln->nsip[i] = 1;
                //aln->sip[i][0] = i;
        }
